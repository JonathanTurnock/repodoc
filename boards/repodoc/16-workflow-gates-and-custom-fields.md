...
- **claude** (2026-07-17T15:55:00.000Z): Dogfooded the content side of the revision — set this board's v2 gates in boards/repodoc/.config.json:1 (review enter = `tests-passing` script, done enter = `peer-review` field checking `reviewed-by = jonathan`), added the `reviewed-by` field, and rewrote the skill and decisions. Left the card in doing until the parser rework lands and the journals are fully backfilled.
